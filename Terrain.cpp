...
bool Terrain::GenerateHeightMap(ID3D11Device* device)
{
	...
	RandomHeightMap();
	SmoothenHeightMap(device, 2); //smoothing after random noise
	NoiseHeightMap(); //2D simplex noise
	SmoothenHeightMap(device, 2); //smoothing after simplex noise
	for (int i = 0; i < 80; i++)
	{
		ParticleDeposition(); //used for creating craters/mountains
	}
	SmoothenHeightMap(device, 2); //crater/mountain smoothing
...
void Terrain::ParticleDeposition()
{
	...
	for (i = 0; i < 500; i++) //1000 iterations
	{
		dir = rand() % 4; 
		if (dir == 2) {
...
void Terrain::ParticleDeposition(ID3D11Device* device)
{
	ParticleDeposition();
	CalculateNormals();
	CalculateTextureCoordinates();
...
{
	int j, k, kk, jj, flag, control = 0;

	control = rand() % 2;
	if (control == 0) //mountains
	{
		flag = 0;
		for (k = -1; k < 2; k++)
			for (j = -1; j < 2; j++)
		...
		if (!flag)
			m_heightMap[x * m_terrainHeight + z].y += maxDisp;
		else
			deposit(x + kk, z + jj);
	}
